# -*- encoding: utf-8 -*-
# stub: middleman-autoprefixer 2.5.0 ruby lib

Gem::Specification.new do |s|
  s.name = "middleman-autoprefixer".freeze
  s.version = "2.5.0"

  s.required_rubygems_version = Gem::Requirement.new(">= 0".freeze) if s.respond_to? :required_rubygems_version=
  s.require_paths = ["lib".freeze]
  s.authors = ["Dominik Porada".freeze, "Thomas Reynolds".freeze]
  s.date = "2015-06-18"
  s.email = ["dominik@porada.co".freeze, "me@tdreyno.com".freeze]
  s.homepage = "https://github.com/middleman/middleman-autoprefixer".freeze
  s.licenses = ["MIT".freeze]
  s.rubygems_version = "2.6.3".freeze
  s.summary = "Autoprefixer integration with Middleman".freeze

  s.installed_by_version = "2.6.3" if s.respond_to? :installed_by_version

  if s.respond_to? :specification_version then
    s.specification_version = 4

    if Gem::Version.new(Gem::VERSION) >= Gem::Version.new('1.2.0') then
      s.add_runtime_dependency(%q<middleman-core>.freeze, [">= 3.3.3"])
      s.add_runtime_dependency(%q<autoprefixer-rails>.freeze, ["~> 5.2.0"])
      s.add_development_dependency(%q<middleman>.freeze, [">= 3.3.3"])
      s.add_development_dependency(%q<cucumber>.freeze, ["~> 1.3"])
      s.add_development_dependency(%q<aruba>.freeze, ["~> 0.6"])
      s.add_development_dependency(%q<coveralls>.freeze, ["~> 0.7"])
      s.add_development_dependency(%q<bundler>.freeze, [">= 1.6"])
      s.add_development_dependency(%q<rake>.freeze, [">= 10.3"])
    else
      s.add_dependency(%q<middleman-core>.freeze, [">= 3.3.3"])
      s.add_dependency(%q<autoprefixer-rails>.freeze, ["~> 5.2.0"])
      s.add_dependency(%q<middleman>.freeze, [">= 3.3.3"])
      s.add_dependency(%q<cucumber>.freeze, ["~> 1.3"])
      s.add_dependency(%q<aruba>.freeze, ["~> 0.6"])
      s.add_dependency(%q<coveralls>.freeze, ["~> 0.7"])
      s.add_dependency(%q<bundler>.freeze, [">= 1.6"])
      s.add_dependency(%q<rake>.freeze, [">= 10.3"])
    end
  else
    s.add_dependency(%q<middleman-core>.freeze, [">= 3.3.3"])
    s.add_dependency(%q<autoprefixer-rails>.freeze, ["~> 5.2.0"])
    s.add_dependency(%q<middleman>.freeze, [">= 3.3.3"])
    s.add_dependency(%q<cucumber>.freeze, ["~> 1.3"])
    s.add_dependency(%q<aruba>.freeze, ["~> 0.6"])
    s.add_dependency(%q<coveralls>.freeze, ["~> 0.7"])
    s.add_dependency(%q<bundler>.freeze, [">= 1.6"])
    s.add_dependency(%q<rake>.freeze, [">= 10.3"])
  end
end
